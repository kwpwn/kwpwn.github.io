...
description: "A mechanism-first, version-aware study of LOLBins: Trust Boundaries, Reachability, and Telemetry, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain LOLBins: Trust Boundaries, Reachability, and Telemetry as a sequence of state transitions and trust boundaries"
...
The subject here is **LOLBins: Trust Boundaries, Reachability, and Telemetry**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 14 in `execution-persistence`; malware-c2 lesson 16 in `execution-persistence`.

## Concept in one sentence

A living-off-the-land binary is a legitimate signed utility whose flexible behavior can cross a security boundary when supplied attacker-controlled input. This definition is deliberately operational: it names
...
1. **State:** A living-off-the-land binary is a legitimate signed utility whose flexible behavior can cross a security boundary when supplied attacker-controlled input.
2. **Boundary:** a catalog entry is a hypothesis, not evidence that every argument works on every build.
3. **Constraint:** application control, network policy, child-process rules, user writable paths, and parent context determine reachability.
4. **Evidence:** detection should model unusual data flow and execution chains rather than blocking filenames without context.
...
For **LOLBins: Trust Boundaries, Reachability, and Telemetry**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **LOLBins: Trust Boundaries, Reachability, and Telemetry** after observing only the first visible
