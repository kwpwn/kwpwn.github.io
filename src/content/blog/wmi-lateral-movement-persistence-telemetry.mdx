...
description: "A mechanism-first, version-aware study of Lateral Movement and Persistence with WMI, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Lateral Movement and Persistence with WMI as a sequence of state transitions and trust boundaries"
...
The subject here is **Lateral Movement and Persistence with WMI**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 13 in `execution-persistence`.

## Concept in one sentence

WMI combines CIM schema, providers, the repository, RPC or DCOM transport, namespaces, and per-namespace security. This definition is deliberately operational: it names
...
1. **State:** WMI combines CIM schema, providers, the repository, RPC or DCOM transport, namespaces, and per-namespace security.
2. **Boundary:** remote method invocation and permanent event subscriptions are different mechanisms with different artifacts.
3. **Constraint:** FilterToConsumerBinding joins an event filter to a consumer and should be examined together with CreatorSID and namespace ACLs.
4. **Evidence:** telemetry spans WMI-Activity logs, process ancestry, RPC/network evidence, repository state, and the privileged identity hosting a provider.
...
For **Lateral Movement and Persistence with WMI**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Lateral Movement and Persistence with WMI** after observing only the first visible
