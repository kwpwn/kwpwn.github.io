...
> **Quick answer:** WMI combines CIM schema, providers, the repository, RPC or DCOM transport, namespaces, and per-namespace security.
...
For **Lateral Movement and Persistence with WMI**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 13 in execution-persistence. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | WMI combines CIM schema, providers, the repository, RPC or DCOM transport, namespaces, and per-namespace security. |
| 2. **Privilege decision** | remote method invocation and permanent event subscriptions are different mechanisms with different artifacts. |
| 3. **Reachability** | FilterToConsumerBinding joins an event filter to a consumer and should be examined together with CreatorSID and namespace ACLs. |
| 4. **Proof** | telemetry spans WMI-Activity logs, process ancestry, RPC/network evidence, repository state, and the privileged identity hosting a provider. |
...
1. **Starting state:** verify this concrete claim — WMI combines CIM schema, providers, the repository, RPC or DCOM transport, namespaces, and per-namespace security.
2. **Decision boundary:** identify and observe the component or access decision
   described here — remote method invocation and permanent event subscriptions are different mechanisms with different artifacts.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — FilterToConsumerBinding joins an event filter to a consumer and should be examined together with CreatorSID and namespace ACLs.
4. **Proof:** demand the joined evidence described here — telemetry spans WMI-Activity logs, process ancestry, RPC/network evidence, repository state, and the privileged identity hosting a provider.
...
- [Lateral Movement and Persistence with WMI](https://yunolay.com/wmi-lateral-persistence/)

## References

1. https://yunolay.com/wmi-lateral-persistence/
