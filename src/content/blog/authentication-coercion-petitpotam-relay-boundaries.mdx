...
description: "A mechanism-first, version-aware study of PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS as a sequence of state transitions and trust boundaries"
...
The subject here is **PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 26 in `lateral-boundaries`.

## Concept in one sentence

Authentication coercion causes a machine or service identity to authenticate to a chosen endpoint but does not by itself grant privilege. This definition is deliberately operational: it names
...
1. **State:** Authentication coercion causes a machine or service identity to authenticate to a chosen endpoint but does not by itself grant privilege.
2. **Boundary:** relay requires a compatible inbound protocol, absent or insufficient channel binding or signing, and a target that accepts the relayed identity for a useful operation.
3. **Constraint:** AD CS web enrollment historically created a high-impact target when template and endpoint conditions aligned.
4. **Evidence:** safe assessment inventories exposure and policy without coercing production identities.
...
For **PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS** after observing only the first visible
