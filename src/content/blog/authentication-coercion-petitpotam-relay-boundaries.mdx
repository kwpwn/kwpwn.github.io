...
> **Quick answer:** Authentication coercion causes a machine or service identity to authenticate to a chosen endpoint but does not by itself grant privilege.
...
For **PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 26 in lateral-boundaries. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | Authentication coercion causes a machine or service identity to authenticate to a chosen endpoint but does not by itself grant privilege. |
| 2. **Privilege decision** | relay requires a compatible inbound protocol, absent or insufficient channel binding or signing, and a target that accepts the relayed identity for a useful operation. |
| 3. **Reachability** | AD CS web enrollment historically created a high-impact target when template and endpoint conditions aligned. |
| 4. **Proof** | safe assessment inventories exposure and policy without coercing production identities. |
...
1. **Starting state:** verify this concrete claim — Authentication coercion causes a machine or service identity to authenticate to a chosen endpoint but does not by itself grant privilege.
2. **Decision boundary:** identify and observe the component or access decision
   described here — relay requires a compatible inbound protocol, absent or insufficient channel binding or signing, and a target that accepts the relayed identity for a useful operation.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — AD CS web enrollment historically created a high-impact target when template and endpoint conditions aligned.
4. **Proof:** demand the joined evidence described here — safe assessment inventories exposure and policy without coercing production identities.
...
- [PetitPotam and Coercion Attacks: Forcing Authentication and Relaying to AD CS](https://yunolay.com/petitpotam-coercion/)

## References

1. https://yunolay.com/petitpotam-coercion/
