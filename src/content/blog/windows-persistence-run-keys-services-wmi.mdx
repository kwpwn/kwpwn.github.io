...
> **Quick answer:** Run keys, startup folders, services, scheduled tasks, WMI subscriptions, COM registration, and browser or Office extensibility invoke different privileged consumers.
...
For **Persistence Techniques on Windows: Run Keys to WMI**, start with one practical question: What changed, which component caused the change, which sensor observed it, and what alternative explanation or sensor gap could produce the same record? The rest
...
The curriculum placement is malware-c2 lesson 14 in execution-persistence. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Observable artifact** | Run keys, startup folders, services, scheduled tasks, WMI subscriptions, COM registration, and browser or Office extensibility invoke different privileged consumers. |
| 2. **Behavior boundary** | persistence is only established when the trigger recurs under the intended identity and survives the expected lifecycle event. |
| 3. **Execution constraint** | per-user and per-machine locations have different permission and visibility models. |
| 4. **Correlated evidence** | an audit records writer, storage location, trigger, consumer, resulting context, telemetry, and removal procedure. |
...
1. **Starting state:** verify this concrete claim — Run keys, startup folders, services, scheduled tasks, WMI subscriptions, COM registration, and browser or Office extensibility invoke different privileged consumers.
2. **Decision boundary:** identify and observe the component or access decision
   described here — persistence is only established when the trigger recurs under the intended identity and survives the expected lifecycle event.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — per-user and per-machine locations have different permission and visibility models.
4. **Proof:** demand the joined evidence described here — an audit records writer, storage location, trigger, consumer, resulting context, telemetry, and removal procedure.
...
- [Persistence Techniques on Windows: Run Keys to WMI](https://yunolay.com/persistence-techniques-on-windows-run-keys-to-wmi/)

## References

1. https://yunolay.com/persistence-techniques-on-windows-run-keys-to-wmi/
