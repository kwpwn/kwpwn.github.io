...
description: "A mechanism-first, version-aware study of Abusing BITS Jobs for Download and Persistence, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Abusing BITS Jobs for Download and Persistence as a sequence of state transitions and trust boundaries"
...
The subject here is **Abusing BITS Jobs for Download and Persistence**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 12 in `execution-persistence`.

## Concept in one sentence

BITS persists transfer jobs through a service-managed queue and can continue work across user sessions and network changes. This definition is deliberately operational: it names
...
1. **State:** BITS persists transfer jobs through a service-managed queue and can continue work across user sessions and network changes.
2. **Boundary:** a job has owner, type, state, files, notification behavior, retry timing, and credentials or proxy context.
3. **Constraint:** asynchronous execution makes process-only narratives incomplete.
4. **Evidence:** defenders correlate BITS client operational events, job enumeration, qmgr storage artifacts, destination paths, notification commands, network destinations, and the creating principal.
...
For **Abusing BITS Jobs for Download and Persistence**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Abusing BITS Jobs for Download and Persistence** after observing only the first visible
