...
> **Quick answer:** Driver objects contain dispatch entry points and reference device objects while device stacks connect bus, function, and filter roles.
...
For **The Windows Driver Model: WDM, KMDF, and Driver Objects**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 19 in io-drivers. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | Driver objects contain dispatch entry points and reference device objects while device stacks connect bus, function, and filter roles. |
| 2. **Manager or boundary** | WDM exposes IRP lifecycle directly, whereas KMDF wraps state, queues, callbacks, object parenting, and synchronization policy. |
| 3. **Versioned constraint** | Plug and Play and power transitions create state beyond create/read/write/device-control dispatch. |
| 4. **Observable evidence** | review should map device interfaces, symbolic links, ACLs, IOCTL access bits, buffering mode, queues, cancellation, and teardown ownership. |
...
| --- | --- |
| **IRP** | An I/O request packet that carries an operation through driver dispatch, completion, cancellation, and teardown. |
| **IOCTL** | A device-control request whose code, access bits, buffering method, input, and output define a driver boundary. |
| **Windows driver model** | The contracts and object lifecycle used to connect drivers, devices, queues, callbacks, Plug and Play, power, and I/O. |
...
1. **Starting state:** verify this concrete claim — Driver objects contain dispatch entry points and reference device objects while device stacks connect bus, function, and filter roles.
2. **Decision boundary:** identify and observe the component or access decision
   described here — WDM exposes IRP lifecycle directly, whereas KMDF wraps state, queues, callbacks, object parenting, and synchronization policy.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — Plug and Play and power transitions create state beyond create/read/write/device-control dispatch.
4. **Proof:** demand the joined evidence described here — review should map device interfaces, symbolic links, ACLs, IOCTL access bits, buffering mode, queues, cancellation, and teardown ownership.
