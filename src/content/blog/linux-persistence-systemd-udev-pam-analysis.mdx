...
description: "A mechanism-first, version-aware study of Linux Persistence: systemd Timers, udev Rules, and PAM Modules, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Linux Persistence: systemd Timers, udev Rules, and PAM Modules as a sequence of state transitions and trust boundaries"
...
The subject here is **Linux Persistence: systemd Timers, udev Rules, and PAM Modules**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 15 in `execution-persistence`.

## Concept in one sentence

systemd units and timers, udev rules, and PAM modules occupy different boot, device, and authentication trust boundaries. This definition is deliberately operational: it names
...
1. **State:** systemd units and timers, udev rules, and PAM modules occupy different boot, device, and authentication trust boundaries.
2. **Boundary:** unit precedence and drop-in directories determine which definition wins.
3. **Constraint:** udev rules run in a constrained service context and should not be treated as an ordinary interactive shell.
4. **Evidence:** PAM changes can affect every authentication path, so analysis must preserve package ownership, module order, file hashes, journal evidence, and recovery access.
...
For **Linux Persistence: systemd Timers, udev Rules, and PAM Modules**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Linux Persistence: systemd Timers, udev Rules, and PAM Modules** after observing only the first visible
