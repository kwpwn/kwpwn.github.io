...
> **Quick answer:** systemd units and timers, udev rules, and PAM modules occupy different boot, device, and authentication trust boundaries.
...
For **Linux Persistence: systemd Timers, udev Rules, and PAM Modules**, start with one practical question: What changed, which component caused the change, which sensor observed it, and what alternative explanation or sensor gap could produce the same record? The rest
...
The curriculum placement is malware-c2 lesson 15 in execution-persistence. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Observable artifact** | systemd units and timers, udev rules, and PAM modules occupy different boot, device, and authentication trust boundaries. |
| 2. **Behavior boundary** | unit precedence and drop-in directories determine which definition wins. |
| 3. **Execution constraint** | udev rules run in a constrained service context and should not be treated as an ordinary interactive shell. |
| 4. **Correlated evidence** | PAM changes can affect every authentication path, so analysis must preserve package ownership, module order, file hashes, journal evidence, and recovery access. |
...
1. **Starting state:** verify this concrete claim — systemd units and timers, udev rules, and PAM modules occupy different boot, device, and authentication trust boundaries.
2. **Decision boundary:** identify and observe the component or access decision
   described here — unit precedence and drop-in directories determine which definition wins.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — udev rules run in a constrained service context and should not be treated as an ordinary interactive shell.
4. **Proof:** demand the joined evidence described here — PAM changes can affect every authentication path, so analysis must preserve package ownership, module order, file hashes, journal evidence, and recovery access.
...
- [Linux Persistence: systemd Timers, udev Rules, and PAM Modules](https://yunolay.com/linux-persistence-systemd-timers-udev-rules-and-pam-modules/)

## References

1. https://yunolay.com/linux-persistence-systemd-timers-udev-rules-and-pam-modules/
