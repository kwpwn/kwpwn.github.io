...
description: "A mechanism-first, version-aware study of Abusing AlwaysInstallElevated for Windows Privilege Escalation, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Abusing AlwaysInstallElevated for Windows Privilege Escalation as a sequence of state transitions and trust boundaries"
...
The subject here is **Abusing AlwaysInstallElevated for Windows Privilege Escalation**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 07 in `service-boundaries`.

## Concept in one sentence

AlwaysInstallElevated becomes dangerous only when both the machine and current-user policy values enable elevated Windows Installer transactions. This definition is deliberately operational: it names
...
1. **State:** AlwaysInstallElevated becomes dangerous only when both the machine and current-user policy values enable elevated Windows Installer transactions.
2. **Boundary:** Windows Installer still parses package metadata, custom actions, repair, and rollback through a complex service boundary.
3. **Constraint:** finding one registry value is an incomplete result and should be reported as a configuration precondition rather than confirmed escalation.
4. **Evidence:** mitigation requires disabling both policy values and auditing software-deployment assumptions.
...
For **Abusing AlwaysInstallElevated for Windows Privilege Escalation**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Abusing AlwaysInstallElevated for Windows Privilege Escalation** after observing only the first visible
