...
> **Quick answer:** AlwaysInstallElevated becomes dangerous only when both the machine and current-user policy values enable elevated Windows Installer transactions.
...
For **Abusing AlwaysInstallElevated for Windows Privilege Escalation**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 07 in service-boundaries. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | AlwaysInstallElevated becomes dangerous only when both the machine and current-user policy values enable elevated Windows Installer transactions. |
| 2. **Privilege decision** | Windows Installer still parses package metadata, custom actions, repair, and rollback through a complex service boundary. |
| 3. **Reachability** | finding one registry value is an incomplete result and should be reported as a configuration precondition rather than confirmed escalation. |
| 4. **Proof** | mitigation requires disabling both policy values and auditing software-deployment assumptions. |
...
1. **Starting state:** verify this concrete claim — AlwaysInstallElevated becomes dangerous only when both the machine and current-user policy values enable elevated Windows Installer transactions.
2. **Decision boundary:** identify and observe the component or access decision
   described here — Windows Installer still parses package metadata, custom actions, repair, and rollback through a complex service boundary.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — finding one registry value is an incomplete result and should be reported as a configuration precondition rather than confirmed escalation.
4. **Proof:** demand the joined evidence described here — mitigation requires disabling both policy values and auditing software-deployment assumptions.
...
- [Abusing AlwaysInstallElevated for Windows Privilege Escalation](https://yunolay.com/always-install-elevated/)
- [AlwaysInstallElevated: MSI Privilege Escalation](https://yunolay.com/alwaysinstallelevated-msi-privilege-escalation/)

## References

1. https://yunolay.com/always-install-elevated/
2. https://yunolay.com/alwaysinstallelevated-msi-privilege-escalation/
