...
description: "A mechanism-first, version-aware study of Application Control with AppLocker and WDAC: Design and Gaps, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Application Control with AppLocker and WDAC: Design and Gaps as a sequence of state transitions and trust boundaries"
...
The subject here is **Application Control with AppLocker and WDAC: Design and Gaps**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 24 in `policy-controls`.

## Concept in one sentence

AppLocker is a rule-based application-control technology while WDAC applies code-integrity policy at a stronger platform boundary. This definition is deliberately operational: it names
...
1. **State:** AppLocker is a rule-based application-control technology while WDAC applies code-integrity policy at a stronger platform boundary.
2. **Boundary:** publisher, path, hash, packaged-app, script, DLL, driver, and managed-installer decisions have different trust implications.
3. **Constraint:** audit mode and enforcement mode must be distinguished in evidence.
4. **Evidence:** a policy review searches for writable trusted paths, overly broad publisher ranges, unmanaged interpreters, missing script or DLL coverage, and deployment paths that cannot sustain enforcement.
...
For **Application Control with AppLocker and WDAC: Design and Gaps**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Application Control with AppLocker and WDAC: Design and Gaps** after observing only the first visible
