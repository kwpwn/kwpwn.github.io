...
description: "A mechanism-first, version-aware study of C2 Framework Architecture: Cobalt Strike, Sliver, and Mythic Compared, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain C2 Framework Architecture: Cobalt Strike, Sliver, and Mythic Compared as a sequence of state transitions and trust boundaries"
...
The subject here is **C2 Framework Architecture: Cobalt Strike, Sliver, and Mythic Compared**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 18 in `c2-operations`.

## Concept in one sentence

Cobalt Strike, Sliver, and Mythic separate operator interface, team server, payload or agent, transport profiles, tasking, and result storage in different ways. This definition is deliberately operational: it names
...
1. **State:** Cobalt Strike, Sliver, and Mythic separate operator interface, team server, payload or agent, transport profiles, tasking, and result storage in different ways.
2. **Boundary:** feature names are not directly comparable unless identity, protocol, execution, extension, and multi-tenant boundaries are aligned.
3. **Constraint:** open-source availability changes reviewability but not the need to validate deployed configuration.
4. **Evidence:** defenders gain more from invariant task and telemetry models than from product-name signatures.
...
For **C2 Framework Architecture: Cobalt Strike, Sliver, and Mythic Compared**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **C2 Framework Architecture: Cobalt Strike, Sliver, and Mythic Compared** after observing only the first visible
