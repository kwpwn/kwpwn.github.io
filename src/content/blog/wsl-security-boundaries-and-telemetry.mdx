...
> **Quick answer:** WSL 1 and WSL 2 have different architecture, kernel, filesystem, and network boundaries.
...
For **Windows Subsystem for Linux (WSL) as an Attack Surface**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 27 in lateral-boundaries. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | WSL 1 and WSL 2 have different architecture, kernel, filesystem, and network boundaries. |
| 2. **Privilege decision** | Windows-to-Linux and Linux-to-Windows interoperability exposes path translation, process launching, environment transfer, and mounted-drive policy. |
| 3. **Reachability** | distribution registration and per-user virtual disks create artifacts outside the usual Windows process narrative. |
| 4. **Proof** | monitoring must correlate wsl.exe and wslhost activity, utility VM or Pico process context, network flows, mounted files, and Windows-side consumers. |
...
1. **Starting state:** verify this concrete claim — WSL 1 and WSL 2 have different architecture, kernel, filesystem, and network boundaries.
2. **Decision boundary:** identify and observe the component or access decision
   described here — Windows-to-Linux and Linux-to-Windows interoperability exposes path translation, process launching, environment transfer, and mounted-drive policy.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — distribution registration and per-user virtual disks create artifacts outside the usual Windows process narrative.
4. **Proof:** demand the joined evidence described here — monitoring must correlate wsl.exe and wslhost activity, utility VM or Pico process context, network flows, mounted files, and Windows-side consumers.
...
- [Windows Subsystem for Linux (WSL) as an Attack Surface](https://yunolay.com/windows-subsystem-for-linux-wsl-as-an-attack-surface/)

## References

1. https://yunolay.com/windows-subsystem-for-linux-wsl-as-an-attack-surface/
