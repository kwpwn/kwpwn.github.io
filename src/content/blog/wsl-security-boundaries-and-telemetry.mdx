...
description: "A mechanism-first, version-aware study of Windows Subsystem for Linux (WSL) as an Attack Surface, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Subsystem for Linux (WSL) as an Attack Surface as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Subsystem for Linux (WSL) as an Attack Surface**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 27 in `lateral-boundaries`.

## Concept in one sentence

WSL 1 and WSL 2 have different architecture, kernel, filesystem, and network boundaries. This definition is deliberately operational: it names
...
1. **State:** WSL 1 and WSL 2 have different architecture, kernel, filesystem, and network boundaries.
2. **Boundary:** Windows-to-Linux and Linux-to-Windows interoperability exposes path translation, process launching, environment transfer, and mounted-drive policy.
3. **Constraint:** distribution registration and per-user virtual disks create artifacts outside the usual Windows process narrative.
4. **Evidence:** monitoring must correlate wsl.exe and wslhost activity, utility VM or Pico process context, network flows, mounted files, and Windows-side consumers.
...
For **Windows Subsystem for Linux (WSL) as an Attack Surface**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Subsystem for Linux (WSL) as an Attack Surface** after observing only the first visible
