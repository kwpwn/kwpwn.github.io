...
> **Quick answer:** Windows exposes process heap APIs over implementations that include the traditional NT heap and the Segment Heap.
...
For **Windows Heap Internals: The NT Heap and Segment Heap**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 12 in memory-manager. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | Windows exposes process heap APIs over implementations that include the traditional NT heap and the Segment Heap. |
| 2. **Manager or boundary** | frontend and backend allocators, size classes, metadata, delayed free, LFH behavior, guard instrumentation, and corruption checks vary by mode and build. |
| 3. **Versioned constraint** | a heap handle does not identify ownership of every pointer without allocation evidence. |
| 4. **Observable evidence** | triage should use page heap or verifier where possible and separate use-after-free, overflow, double free, and metadata symptom. |
...
1. **Starting state:** verify this concrete claim — Windows exposes process heap APIs over implementations that include the traditional NT heap and the Segment Heap.
2. **Decision boundary:** identify and observe the component or access decision
   described here — frontend and backend allocators, size classes, metadata, delayed free, LFH behavior, guard instrumentation, and corruption checks vary by mode and build.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — a heap handle does not identify ownership of every pointer without allocation evidence.
4. **Proof:** demand the joined evidence described here — triage should use page heap or verifier where possible and separate use-after-free, overflow, double free, and metadata symptom.
...
- [Windows Heap Internals: The NT Heap and Segment Heap](https://yunolay.com/windows-heap-internals-nt-heap-segment-heap/)

## References

1. https://yunolay.com/windows-heap-internals-nt-heap-segment-heap/
