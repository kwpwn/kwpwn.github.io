...
> **Quick answer:** Interrupt request levels prioritize interrupt and synchronization work but are not thread privilege levels.
...
For **Interrupts, IRQLs, DPCs, and APCs**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 30 in kernel-platform. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | Interrupt request levels prioritize interrupt and synchronization work but are not thread privilege levels. |
| 2. **Manager or boundary** | an ISR performs minimal device-specific work and usually schedules a DPC for deferred processing. |
| 3. **Versioned constraint** | APCs execute in a thread context and have kernel, special-kernel, and user-mode delivery rules. |
| 4. **Observable evidence** | correctness depends on IRQL, pageable-code restrictions, lock ordering, CPU locality, interrupt affinity, and object lifetime across deferred callbacks. |
...
| **DPC** | Deferred kernel work scheduled after a high-priority interrupt routine finishes its time-critical portion. |
...
1. **Starting state:** verify this concrete claim — Interrupt request levels prioritize interrupt and synchronization work but are not thread privilege levels.
2. **Decision boundary:** identify and observe the component or access decision
   described here — an ISR performs minimal device-specific work and usually schedules a DPC for deferred processing.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — APCs execute in a thread context and have kernel, special-kernel, and user-mode delivery rules.
4. **Proof:** demand the joined evidence described here — correctness depends on IRQL, pageable-code restrictions, lock ordering, CPU locality, interrupt affinity, and object lifetime across deferred callbacks.
...
- [Interrupts, IRQLs, DPCs, and APCs](https://yunolay.com/interrupts-irqls-dpcs-and-apcs/)

## References

1. https://yunolay.com/interrupts-irqls-dpcs-and-apcs/
