...
description: "A mechanism-first, version-aware study of Interrupts, IRQLs, DPCs, and APCs, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Interrupts, IRQLs, DPCs, and APCs as a sequence of state transitions and trust boundaries"
...
The subject here is **Interrupts, IRQLs, DPCs, and APCs**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 30 in `kernel-platform`.

## Concept in one sentence

Interrupt request levels prioritize interrupt and synchronization work but are not thread privilege levels. This definition is deliberately operational: it names
...
1. **State:** Interrupt request levels prioritize interrupt and synchronization work but are not thread privilege levels.
2. **Boundary:** an ISR performs minimal device-specific work and usually schedules a DPC for deferred processing.
3. **Constraint:** APCs execute in a thread context and have kernel, special-kernel, and user-mode delivery rules.
4. **Evidence:** correctness depends on IRQL, pageable-code restrictions, lock ordering, CPU locality, interrupt affinity, and object lifetime across deferred callbacks.
...
For **Interrupts, IRQLs, DPCs, and APCs**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Interrupts, IRQLs, DPCs, and APCs** after observing only the first visible
