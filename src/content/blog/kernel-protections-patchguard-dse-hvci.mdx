...
description: "A mechanism-first, version-aware study of Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI as a sequence of state transitions and trust boundaries"
...
The subject here is **Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 31 in `kernel-platform`.

## Concept in one sentence

Driver Signature Enforcement controls which kernel images may load, HVCI moves selected code-integrity decisions into a virtualization-protected trust level, and PatchGuard detects unsupported modification of critical kernel state. This definition is deliberately operational: it names
...
1. **State:** Driver Signature Enforcement controls which kernel images may load, HVCI moves selected code-integrity decisions into a virtualization-protected trust level, and PatchGuard detects unsupported modification of critical kernel state.
2. **Boundary:** these controls have different activation, hardware, policy, and compatibility requirements.
3. **Constraint:** a single registry value does not prove effective enforcement.
4. **Evidence:** validation combines System Information, code-integrity logs, Device Guard state, boot configuration, loaded drivers, and supported Microsoft diagnostics.
...
For **Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI** after observing only the first visible
