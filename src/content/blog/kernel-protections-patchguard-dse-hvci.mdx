...
> **Quick answer:** Driver Signature Enforcement controls which kernel images may load, HVCI moves selected code-integrity decisions into a virtualization-protected trust level, and PatchGuard detects unsupported modification of critical kernel state.
...
For **Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 31 in kernel-platform. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | Driver Signature Enforcement controls which kernel images may load, HVCI moves selected code-integrity decisions into a virtualization-protected trust level, and PatchGuard detects unsupported modification of critical kernel state. |
| 2. **Manager or boundary** | these controls have different activation, hardware, policy, and compatibility requirements. |
| 3. **Versioned constraint** | a single registry value does not prove effective enforcement. |
| 4. **Observable evidence** | validation combines System Information, code-integrity logs, Device Guard state, boot configuration, loaded drivers, and supported Microsoft diagnostics. |
...
| --- | --- |
| **PatchGuard** | Kernel Patch Protection, which detects unsupported modification of selected critical kernel state. |
| **HVCI** | Hypervisor-protected code integrity, which moves selected code-integrity decisions behind a VBS trust boundary. |
...
1. **Starting state:** verify this concrete claim — Driver Signature Enforcement controls which kernel images may load, HVCI moves selected code-integrity decisions into a virtualization-protected trust level, and PatchGuard detects unsupported modification of critical kernel state.
2. **Decision boundary:** identify and observe the component or access decision
   described here — these controls have different activation, hardware, policy, and compatibility requirements.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — a single registry value does not prove effective enforcement.
4. **Proof:** demand the joined evidence described here — validation combines System Information, code-integrity logs, Device Guard state, boot configuration, loaded drivers, and supported Microsoft diagnostics.
...
- [Modern Windows Kernel Protections: PatchGuard, DSE, and HVCI](https://yunolay.com/modern-windows-kernel-protections-patchguard-dse-hvci/)

## References

1. https://yunolay.com/modern-windows-kernel-protections-patchguard-dse-hvci/
