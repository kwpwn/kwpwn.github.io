...
description: "A mechanism-first, version-aware study of Windows Shellcode: Writing and Analyzing Position-Independent Payloads, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Shellcode: Writing and Analyzing Position-Independent Payloads as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Shellcode: Writing and Analyzing Position-Independent Payloads**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 09 in `memory-execution`.

## Concept in one sentence

Position-independent code cannot assume a fixed image base and therefore resolves data, APIs, or imports through relative addressing or runtime discovery. This definition is deliberately operational: it names
...
1. **State:** Position-independent code cannot assume a fixed image base and therefore resolves data, APIs, or imports through relative addressing or runtime discovery.
2. **Boundary:** x64 stack alignment, shadow space, register volatility, exception metadata, and calling convention still apply.
3. **Constraint:** analysis separates decoder, resolver, configuration, transport, and final capability stages.
4. **Evidence:** safe exercises use shellcode-shaped byte arrays that return a constant and never resolve networking, process creation, or persistence APIs.
...
For **Windows Shellcode: Writing and Analyzing Position-Independent Payloads**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Shellcode: Writing and Analyzing Position-Independent Payloads** after observing only the first visible
