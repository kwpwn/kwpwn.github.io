...
> **Quick answer:** COM activation resolves a class identifier through registration, activation context, bitness, and server type before a client receives an interface pointer.
...
For **COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 10 in execution-persistence; malware-c2 lesson 13 in execution-persistence. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | COM activation resolves a class identifier through registration, activation context, bitness, and server type before a client receives an interface pointer. |
| 2. **Privilege decision** | per-user registration can shadow machine-wide registration in some activation paths but not every CLSID or packaged scenario. |
| 3. **Reachability** | in-process servers inherit the client's token while local servers have their own launch and access security. |
| 4. **Proof** | a complete finding proves a real client, resolution precedence, writable registration, trigger, and resulting execution context. |
...
1. **Starting state:** verify this concrete claim — COM activation resolves a class identifier through registration, activation context, bitness, and server type before a client receives an interface pointer.
2. **Decision boundary:** identify and observe the component or access decision
   described here — per-user registration can shadow machine-wide registration in some activation paths but not every CLSID or packaged scenario.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — in-process servers inherit the client's token while local servers have their own launch and access security.
4. **Proof:** demand the joined evidence described here — a complete finding proves a real client, resolution precedence, writable registration, trigger, and resulting execution context.
...
- [COM Hijacking for Persistence and Privilege Escalation](https://yunolay.com/com-hijacking-for-persistence-and-privilege-escalation/)
- [COM Hijacking for Persistence and Privilege Escalation](https://yunolay.com/com-hijacking-for-persistence-and-privilege-escalation/)

## References

1. https://yunolay.com/com-hijacking-for-persistence-and-privilege-escalation/
