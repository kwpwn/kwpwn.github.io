...
description: "A mechanism-first, version-aware study of COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry as a sequence of state transitions and trust boundaries"
...
The subject here is **COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 10 in `execution-persistence`; malware-c2 lesson 13 in `execution-persistence`.

## Concept in one sentence

COM activation resolves a class identifier through registration, activation context, bitness, and server type before a client receives an interface pointer. This definition is deliberately operational: it names
...
1. **State:** COM activation resolves a class identifier through registration, activation context, bitness, and server type before a client receives an interface pointer.
2. **Boundary:** per-user registration can shadow machine-wide registration in some activation paths but not every CLSID or packaged scenario.
3. **Constraint:** in-process servers inherit the client's token while local servers have their own launch and access security.
4. **Evidence:** a complete finding proves a real client, resolution precedence, writable registration, trigger, and resulting execution context.
...
For **COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **COM Activation and Hijacking: Resolution, Security Boundaries, and Telemetry** after observing only the first visible
