...
description: "A mechanism-first, version-aware study of Process Injection Internals: DLL Injection, Reflective Loading, and Process Hollowing, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Process Injection Internals: DLL Injection, Reflective Loading, and Process Hollowing as a sequence of state transitions and trust boundaries"
...
The subject here is **Process Injection Internals: DLL Injection, Reflective Loading, and Process Hollowing**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 08 in `memory-execution`.

## Concept in one sentence

Process injection is a set of memory, handle, thread, section, APC, and loader state transitions rather than a single API sequence. This definition is deliberately operational: it names
...
1. **State:** Process injection is a set of memory, handle, thread, section, APC, and loader state transitions rather than a single API sequence.
2. **Boundary:** classic remote-thread injection, section mapping, process hollowing, reflective loading, and thread-context manipulation leave different artifacts.
3. **Constraint:** access rights and target architecture constrain each chain.
4. **Evidence:** detection should combine cross-process handle activity, executable-memory provenance, image and thread start relationships, and target-process expectations.
...
For **Process Injection Internals: DLL Injection, Reflective Loading, and Process Hollowing**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Process Injection Internals: DLL Injection, Reflective Loading, and Process Hollowing** after observing only the first visible
