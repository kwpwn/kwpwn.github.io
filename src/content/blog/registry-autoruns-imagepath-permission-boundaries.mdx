...
> **Quick answer:** An autorun location matters only when a more privileged consumer reads it and the lower-privileged actor can change the effective command or a dependency.
...
For **Abusing Registry Autoruns for Windows Persistence and Privilege Escalation**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 11 in execution-persistence. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | An autorun location matters only when a more privileged consumer reads it and the lower-privileged actor can change the effective command or a dependency. |
| 2. **Privilege decision** | registry-key permissions, value permissions inherited through the key, filesystem ACLs, environment expansion, quoting, and command parsing all contribute. |
| 3. **Reachability** | service ImagePath is controlled through SCM operations and registry representation should not be treated as an ordinary autorun value. |
| 4. **Proof** | defenders should capture the old and new value, writer identity, consumer context, and trigger. |
...
| --- | --- |
| **Service Control Manager** | The privileged manager that stores service configuration, starts service processes, and authorizes service operations. |
...
1. **Starting state:** verify this concrete claim — An autorun location matters only when a more privileged consumer reads it and the lower-privileged actor can change the effective command or a dependency.
2. **Decision boundary:** identify and observe the component or access decision
   described here — registry-key permissions, value permissions inherited through the key, filesystem ACLs, environment expansion, quoting, and command parsing all contribute.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — service ImagePath is controlled through SCM operations and registry representation should not be treated as an ordinary autorun value.
4. **Proof:** demand the joined evidence described here — defenders should capture the old and new value, writer identity, consumer context, and trigger.
...
- [Abusing Registry Autoruns for Windows Persistence and Privilege Escalation](https://yunolay.com/registry-autoruns-abuse/)
- [Insecure Registry Autoruns and Service ImagePath Writes](https://yunolay.com/insecure-registry-autoruns-and-service-imagepath-writes/)

## References

1. https://yunolay.com/registry-autoruns-abuse/
2. https://yunolay.com/insecure-registry-autoruns-and-service-imagepath-writes/
