...
description: "A mechanism-first, version-aware study of Abusing Registry Autoruns for Windows Persistence and Privilege Escalation, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Abusing Registry Autoruns for Windows Persistence and Privilege Escalation as a sequence of state transitions and trust boundaries"
...
The subject here is **Abusing Registry Autoruns for Windows Persistence and Privilege Escalation**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 11 in `execution-persistence`.

## Concept in one sentence

An autorun location matters only when a more privileged consumer reads it and the lower-privileged actor can change the effective command or a dependency. This definition is deliberately operational: it names
...
1. **State:** An autorun location matters only when a more privileged consumer reads it and the lower-privileged actor can change the effective command or a dependency.
2. **Boundary:** registry-key permissions, value permissions inherited through the key, filesystem ACLs, environment expansion, quoting, and command parsing all contribute.
3. **Constraint:** service ImagePath is controlled through SCM operations and registry representation should not be treated as an ordinary autorun value.
4. **Evidence:** defenders should capture the old and new value, writer identity, consumer context, and trigger.
...
For **Abusing Registry Autoruns for Windows Persistence and Privilege Escalation**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Abusing Registry Autoruns for Windows Persistence and Privilege Escalation** after observing only the first visible
