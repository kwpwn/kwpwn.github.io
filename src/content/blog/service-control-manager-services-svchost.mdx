...
description: "A mechanism-first, version-aware study of Windows Services and the Service Control Manager, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Services and the Service Control Manager as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Services and the Service Control Manager**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 21 in `ipc-services`.

## Concept in one sentence

The Service Control Manager maintains a service database, creates service processes, sends control requests, and enforces service-object access. This definition is deliberately operational: it names
...
1. **State:** The Service Control Manager maintains a service database, creates service processes, sends control requests, and enforces service-object access.
2. **Boundary:** ImagePath, ServiceDll, account, start mode, dependencies, failure actions, triggers, and required privileges affect lifecycle.
3. **Constraint:** svchost groups multiple service DLLs according to configuration and modern builds may split services for isolation.
4. **Evidence:** analysis separates SCM metadata, registry representation, process token, service SID, pipe or RPC interfaces, and binary ACLs.
...
For **Windows Services and the Service Control Manager**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Services and the Service Control Manager** after observing only the first visible
