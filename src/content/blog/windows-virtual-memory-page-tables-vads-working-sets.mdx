...
description: "A mechanism-first, version-aware study of Windows Virtual Memory: Page Tables, VADs, and Working Sets, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Virtual Memory: Page Tables, VADs, and Working Sets as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Virtual Memory: Page Tables, VADs, and Working Sets**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 10 in `memory-manager`.

## Concept in one sentence

A process address space is described through regions and VADs while hardware page tables translate individual virtual pages. This definition is deliberately operational: it names
...
1. **State:** A process address space is described through regions and VADs while hardware page tables translate individual virtual pages.
2. **Boundary:** valid, transition, prototype, demand-zero, pagefile-backed, and copy-on-write states describe different ownership and residency conditions.
3. **Constraint:** the working set is the resident subset currently charged to a process, not its entire committed address space.
4. **Evidence:** investigations should correlate VAD metadata, PTE state, section backing, protection, commit, and page provenance.
...
For **Windows Virtual Memory: Page Tables, VADs, and Working Sets**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Virtual Memory: Page Tables, VADs, and Working Sets** after observing only the first visible
