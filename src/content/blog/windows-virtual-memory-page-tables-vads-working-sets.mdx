...
> **Quick answer:** A process address space is described through regions and VADs while hardware page tables translate individual virtual pages.
...
For **Windows Virtual Memory: Page Tables, VADs, and Working Sets**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 10 in memory-manager. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | A process address space is described through regions and VADs while hardware page tables translate individual virtual pages. |
| 2. **Manager or boundary** | valid, transition, prototype, demand-zero, pagefile-backed, and copy-on-write states describe different ownership and residency conditions. |
| 3. **Versioned constraint** | the working set is the resident subset currently charged to a process, not its entire committed address space. |
| 4. **Observable evidence** | investigations should correlate VAD metadata, PTE state, section backing, protection, commit, and page provenance. |
...
| --- | --- |
| **VAD** | A kernel descriptor for a process virtual-address region, including range, protection, and backing relationships. |
| **Page table entry** | The hardware-visible translation and state record for one virtual page. |
...
1. **Starting state:** verify this concrete claim — A process address space is described through regions and VADs while hardware page tables translate individual virtual pages.
2. **Decision boundary:** identify and observe the component or access decision
   described here — valid, transition, prototype, demand-zero, pagefile-backed, and copy-on-write states describe different ownership and residency conditions.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — the working set is the resident subset currently charged to a process, not its entire committed address space.
4. **Proof:** demand the joined evidence described here — investigations should correlate VAD metadata, PTE state, section backing, protection, commit, and page provenance.
...
- [Windows Virtual Memory: Page Tables, PTEs, and Working Sets](https://yunolay.com/windows-virtual-memory-page-tables-ptes-working-sets/)

## References

1. https://yunolay.com/windows-virtual-memory-page-tables-ptes-working-sets/
