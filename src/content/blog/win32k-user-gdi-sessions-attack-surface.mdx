...
> **Quick answer:** The GUI subsystem spans user-mode libraries, CSRSS-related responsibilities, session state, desktop and window-station objects, and win32k kernel components for USER and GDI operations.
...
For **The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 32 in kernel-platform. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | The GUI subsystem spans user-mode libraries, CSRSS-related responsibilities, session state, desktop and window-station objects, and win32k kernel components for USER and GDI operations. |
| 2. **Manager or boundary** | handles in USER and GDI tables are not ordinary Object Manager handles. |
| 3. **Versioned constraint** | message dispatch, callbacks, composition, fonts, graphics drivers, and shared data create complex cross-boundary flows. |
| 4. **Observable evidence** | modern mitigations, process win32k lockdown, session isolation, and build changes alter reachability. |
...
1. **Starting state:** verify this concrete claim — The GUI subsystem spans user-mode libraries, CSRSS-related responsibilities, session state, desktop and window-station objects, and win32k kernel components for USER and GDI operations.
2. **Decision boundary:** identify and observe the component or access decision
   described here — handles in USER and GDI tables are not ordinary Object Manager handles.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — message dispatch, callbacks, composition, fonts, graphics drivers, and shared data create complex cross-boundary flows.
4. **Proof:** demand the joined evidence described here — modern mitigations, process win32k lockdown, session isolation, and build changes alter reachability.
...
- [The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface](https://yunolay.com/win32k-gui-subsystem-user-gdi-kernel-attack-surface/)

## References

1. https://yunolay.com/win32k-gui-subsystem-user-gdi-kernel-attack-surface/
