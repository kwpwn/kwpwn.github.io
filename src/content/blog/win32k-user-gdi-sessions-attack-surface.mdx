...
description: "A mechanism-first, version-aware study of The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface as a sequence of state transitions and trust boundaries"
...
The subject here is **The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 32 in `kernel-platform`.

## Concept in one sentence

The GUI subsystem spans user-mode libraries, CSRSS-related responsibilities, session state, desktop and window-station objects, and win32k kernel components for USER and GDI operations. This definition is deliberately operational: it names
...
1. **State:** The GUI subsystem spans user-mode libraries, CSRSS-related responsibilities, session state, desktop and window-station objects, and win32k kernel components for USER and GDI operations.
2. **Boundary:** handles in USER and GDI tables are not ordinary Object Manager handles.
3. **Constraint:** message dispatch, callbacks, composition, fonts, graphics drivers, and shared data create complex cross-boundary flows.
4. **Evidence:** modern mitigations, process win32k lockdown, session isolation, and build changes alter reachability.
...
For **The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **The win32k GUI Subsystem: USER, GDI, and the Kernel Attack Surface** after observing only the first visible
