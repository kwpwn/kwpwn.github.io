...
description: "A mechanism-first, version-aware study of The Filter Manager and Minifilter Drivers, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain The Filter Manager and Minifilter Drivers as a sequence of state transitions and trust boundaries"
...
The subject here is **The Filter Manager and Minifilter Drivers**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 20 in `io-drivers`.

## Concept in one sentence

Filter Manager orders minifilters by altitude and instances within file-system stacks. This definition is deliberately operational: it names
...
1. **State:** Filter Manager orders minifilters by altitude and instances within file-system stacks.
2. **Boundary:** pre-operation and post-operation callbacks can observe or modify selected operations while contexts attach state to volumes, instances, files, streams, handles, or transactions.
3. **Constraint:** callback IRQL, reentrancy, name queries, cancellation, and teardown create subtle lifetime rules.
4. **Evidence:** security review maps registrations, altitude, communication ports, message schema, bypass paths, and generated I/O.
...
For **The Filter Manager and Minifilter Drivers**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **The Filter Manager and Minifilter Drivers** after observing only the first visible
