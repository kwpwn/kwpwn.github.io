...
description: "A mechanism-first, version-aware study of Processes and the EPROCESS Structure, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Processes and the EPROCESS Structure as a sequence of state transitions and trust boundaries"
...
The subject here is **Processes and the EPROCESS Structure**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 02 in `processes-execution`.

## Concept in one sentence

EPROCESS is a version-specific executive process object that references address-space, token, handle-table, job, security, and scheduling state rather than defining a stable public ABI. This definition is deliberately operational: it names
...
1. **State:** EPROCESS is a version-specific executive process object that references address-space, token, handle-table, job, security, and scheduling state rather than defining a stable public ABI.
2. **Boundary:** creation and teardown proceed through multiple callbacks and reference-counted lifetimes.
3. **Constraint:** process identifiers and active-process lists are indexes, not ownership guarantees.
4. **Evidence:** debugging must use matching symbols and distinguish live process object, exiting process, retained handle, and reused identifier.
...
For **Processes and the EPROCESS Structure**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Processes and the EPROCESS Structure** after observing only the first visible
