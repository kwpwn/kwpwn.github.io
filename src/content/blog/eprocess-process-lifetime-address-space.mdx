...
> **Quick answer:** EPROCESS is a version-specific executive process object that references address-space, token, handle-table, job, security, and scheduling state rather than defining a stable public ABI.
...
For **Processes and the EPROCESS Structure**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 02 in processes-execution. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | EPROCESS is a version-specific executive process object that references address-space, token, handle-table, job, security, and scheduling state rather than defining a stable public ABI. |
| 2. **Manager or boundary** | creation and teardown proceed through multiple callbacks and reference-counted lifetimes. |
| 3. **Versioned constraint** | process identifiers and active-process lists are indexes, not ownership guarantees. |
| 4. **Observable evidence** | debugging must use matching symbols and distinguish live process object, exiting process, retained handle, and reused identifier. |
...
1. **Starting state:** verify this concrete claim — EPROCESS is a version-specific executive process object that references address-space, token, handle-table, job, security, and scheduling state rather than defining a stable public ABI.
2. **Decision boundary:** identify and observe the component or access decision
   described here — creation and teardown proceed through multiple callbacks and reference-counted lifetimes.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — process identifiers and active-process lists are indexes, not ownership guarantees.
4. **Proof:** demand the joined evidence described here — debugging must use matching symbols and distinguish live process object, exiting process, retained handle, and reused identifier.
...
- [Processes and the EPROCESS Structure](https://yunolay.com/processes-and-the-eprocess-structure/)

## References

1. https://yunolay.com/processes-and-the-eprocess-structure/
