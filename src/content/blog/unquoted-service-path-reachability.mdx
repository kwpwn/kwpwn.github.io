...
description: "A mechanism-first, version-aware study of Privilege Escalation via Unquoted Service Paths on Windows, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Privilege Escalation via Unquoted Service Paths on Windows as a sequence of state transitions and trust boundaries"
...
The subject here is **Privilege Escalation via Unquoted Service Paths on Windows**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 06 in `service-boundaries`.

## Concept in one sentence

An unquoted service ImagePath containing spaces can be split into candidate executable paths by Windows command-line resolution. This definition is deliberately operational: it names
...
1. **State:** An unquoted service ImagePath containing spaces can be split into candidate executable paths by Windows command-line resolution.
2. **Boundary:** exploitability additionally requires that a standard user can create the earliest reachable candidate and that the service can be restarted or will start later.
3. **Constraint:** arguments, environment expansion, existing directories, service account, and filesystem inheritance determine the real path.
4. **Evidence:** validation should enumerate candidates and ACLs without planting an executable.
...
For **Privilege Escalation via Unquoted Service Paths on Windows**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Privilege Escalation via Unquoted Service Paths on Windows** after observing only the first visible
