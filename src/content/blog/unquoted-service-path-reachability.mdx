...
> **Quick answer:** An unquoted service ImagePath containing spaces can be split into candidate executable paths by Windows command-line resolution.
...
For **Privilege Escalation via Unquoted Service Paths on Windows**, start with one practical question: Which lower-privileged input can reach which privileged consumer, under what exact configuration, and what evidence would disprove the chain? The rest
...
The curriculum placement is windows-privesc lesson 06 in service-boundaries. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Starting state** | An unquoted service ImagePath containing spaces can be split into candidate executable paths by Windows command-line resolution. |
| 2. **Privilege decision** | exploitability additionally requires that a standard user can create the earliest reachable candidate and that the service can be restarted or will start later. |
| 3. **Reachability** | arguments, environment expansion, existing directories, service account, and filesystem inheritance determine the real path. |
| 4. **Proof** | validation should enumerate candidates and ACLs without planting an executable. |
...
1. **Starting state:** verify this concrete claim — An unquoted service ImagePath containing spaces can be split into candidate executable paths by Windows command-line resolution.
2. **Decision boundary:** identify and observe the component or access decision
   described here — exploitability additionally requires that a standard user can create the earliest reachable candidate and that the service can be restarted or will start later.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — arguments, environment expansion, existing directories, service account, and filesystem inheritance determine the real path.
4. **Proof:** demand the joined evidence described here — validation should enumerate candidates and ACLs without planting an executable.
...
- [Privilege Escalation via Unquoted Service Paths on Windows](https://yunolay.com/unquoted-service-path/)
- [Unquoted Service Path Exploitation on Windows](https://yunolay.com/unquoted-service-path-exploitation-on-windows/)

## References

1. https://yunolay.com/unquoted-service-path/
2. https://yunolay.com/unquoted-service-path-exploitation-on-windows/
