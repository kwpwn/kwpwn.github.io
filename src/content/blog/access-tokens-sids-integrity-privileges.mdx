...
description: "A mechanism-first, version-aware study of Windows Access Tokens: SIDs, Integrity Levels, Privileges, and DACLs, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Access Tokens: SIDs, Integrity Levels, Privileges, and DACLs as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Access Tokens: SIDs, Integrity Levels, Privileges, and DACLs**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 02 in `foundations`; windows-internals lesson 16 in `security-objects`.

## Concept in one sentence

A token carries user and group SIDs, enabled and deny-only attributes, privileges, integrity information, virtualization state, and restriction data. This definition is deliberately operational: it names
...
1. **State:** A token carries user and group SIDs, enabled and deny-only attributes, privileges, integrity information, virtualization state, and restriction data.
2. **Boundary:** object access combines the caller token, requested access, generic mapping, the object's security descriptor, mandatory policy, and any privilege-assisted path.
3. **Constraint:** a primary token supplies a process-wide default while an impersonation token can change a thread's effective security context.
4. **Evidence:** authorization evidence must record the exact token, object, access mask, and access-check result instead of reducing the outcome to an account name.
...
For **Windows Access Tokens: SIDs, Integrity Levels, Privileges, and DACLs**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Access Tokens: SIDs, Integrity Levels, Privileges, and DACLs** after observing only the first visible
