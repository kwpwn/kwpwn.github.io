...
description: "A mechanism-first, version-aware study of LSA Internals: Authentication, Logon Sessions, and Credential Guard, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain LSA Internals: Authentication, Logon Sessions, and Credential Guard as a sequence of state transitions and trust boundaries"
...
The subject here is **LSA Internals: Authentication, Logon Sessions, and Credential Guard**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 17 in `security-objects`.

## Concept in one sentence

LSA policy, authentication packages, credential providers, Winlogon, LSASS, SAM or Active Directory, and Security Support Providers have separate roles. This definition is deliberately operational: it names
...
1. **State:** LSA policy, authentication packages, credential providers, Winlogon, LSASS, SAM or Active Directory, and Security Support Providers have separate roles.
2. **Boundary:** a successful authentication creates a logon session and token through a specific logon type and package.
3. **Constraint:** cached interactive logon, network authentication, delegation, and service logon retain different material.
4. **Evidence:** Credential Guard and LSA protection change storage and process access but do not remove every credential or delegation risk.
...
For **LSA Internals: Authentication, Logon Sessions, and Credential Guard**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **LSA Internals: Authentication, Logon Sessions, and Credential Guard** after observing only the first visible
