...
> **Quick answer:** LSA policy, authentication packages, credential providers, Winlogon, LSASS, SAM or Active Directory, and Security Support Providers have separate roles.
...
For **LSA Internals: Authentication, Logon Sessions, and Credential Guard**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 17 in security-objects. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | LSA policy, authentication packages, credential providers, Winlogon, LSASS, SAM or Active Directory, and Security Support Providers have separate roles. |
| 2. **Manager or boundary** | a successful authentication creates a logon session and token through a specific logon type and package. |
| 3. **Versioned constraint** | cached interactive logon, network authentication, delegation, and service logon retain different material. |
| 4. **Observable evidence** | Credential Guard and LSA protection change storage and process access but do not remove every credential or delegation risk. |
...
1. **Starting state:** verify this concrete claim — LSA policy, authentication packages, credential providers, Winlogon, LSASS, SAM or Active Directory, and Security Support Providers have separate roles.
2. **Decision boundary:** identify and observe the component or access decision
   described here — a successful authentication creates a logon session and token through a specific logon type and package.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — cached interactive logon, network authentication, delegation, and service logon retain different material.
4. **Proof:** demand the joined evidence described here — Credential Guard and LSA protection change storage and process access but do not remove every credential or delegation risk.
...
- [LSA Internals: Authentication, Logon Sessions, and Credential Guard](https://yunolay.com/lsa-internals-authentication-logon-sessions-credential-guard/)

## References

1. https://yunolay.com/lsa-internals-authentication-logon-sessions-credential-guard/
