...
description: "A mechanism-first, version-aware study of Practical Windows Enumeration with winPEAS, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Practical Windows Enumeration with winPEAS as a sequence of state transitions and trust boundaries"
...
The subject here is **Practical Windows Enumeration with winPEAS**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 03 in `discovery-evidence`.

## Concept in one sentence

Enumeration is hypothesis generation, not proof of exploitation. This definition is deliberately operational: it names
...
1. **State:** Enumeration is hypothesis generation, not proof of exploitation.
2. **Boundary:** automated findings must be reduced to actor control, a privileged consumer, a reachable trigger, and a measurable privileged sink.
3. **Constraint:** access checks should be performed as the actual standard-user token because administrator-shell results create false positives.
4. **Evidence:** a useful evidence bundle preserves command lines, timestamps, ACLs, service configuration, token state, and a negative control.
...
For **Practical Windows Enumeration with winPEAS**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Practical Windows Enumeration with winPEAS** after observing only the first visible
