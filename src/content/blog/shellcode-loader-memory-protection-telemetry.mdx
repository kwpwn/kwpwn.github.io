...
description: "A mechanism-first, version-aware study of Shellcode Loaders and Common Evasion Patterns, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Shellcode Loaders and Common Evasion Patterns as a sequence of state transitions and trust boundaries"
...
The subject here is **Shellcode Loaders and Common Evasion Patterns**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as malware-c2 lesson 10 in `memory-execution`.

## Concept in one sentence

A loader controls allocation, content placement, relocation or resolution, memory protection, execution transfer, and cleanup. This definition is deliberately operational: it names
...
1. **State:** A loader controls allocation, content placement, relocation or resolution, memory protection, execution transfer, and cleanup.
2. **Boundary:** executable private memory is a useful signal but legitimate runtimes also create it.
3. **Constraint:** staged decryption and indirect execution shift the order of observable transitions without removing them.
4. **Evidence:** safe analysis uses inert buffers and measures VirtualAlloc or section state, protection changes, thread starts, call stacks, and image provenance.
...
For **Shellcode Loaders and Common Evasion Patterns**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Shellcode Loaders and Common Evasion Patterns** after observing only the first visible
