...
description: "A mechanism-first, version-aware study of WoW64: Running 32-bit Processes on 64-bit Windows, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain WoW64: Running 32-bit Processes on 64-bit Windows as a sequence of state transitions and trust boundaries"
...
The subject here is **WoW64: Running 32-bit Processes on 64-bit Windows**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 26 in `telemetry-runtime`.

## Concept in one sentence

WoW64 combines a 32-bit user-mode environment with translation and a native 64-bit kernel. This definition is deliberately operational: it names
...
1. **State:** WoW64 combines a 32-bit user-mode environment with translation and a native 64-bit kernel.
2. **Boundary:** a 32-bit process has architecture-specific ntdll behavior and transitions through WoW64 support before native system service dispatch.
3. **Constraint:** filesystem and registry redirection preserve compatibility but can be disabled or bypassed in defined cases.
4. **Evidence:** analysis must label pointer width, structure layout, module view, system directory, registry view, and debugger architecture.
...
For **WoW64: Running 32-bit Processes on 64-bit Windows**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **WoW64: Running 32-bit Processes on 64-bit Windows** after observing only the first visible
