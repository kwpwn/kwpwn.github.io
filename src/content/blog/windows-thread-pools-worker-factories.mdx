...
description: "A mechanism-first, version-aware study of Windows Thread Pools, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain Windows Thread Pools as a sequence of state transitions and trust boundaries"
...
The subject here is **Windows Thread Pools**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-internals lesson 05 in `processes-execution`.

## Concept in one sentence

The Windows thread-pool API schedules work, timers, waits, and asynchronous I/O over managed worker threads. This definition is deliberately operational: it names
...
1. **State:** The Windows thread-pool API schedules work, timers, waits, and asynchronous I/O over managed worker threads.
2. **Boundary:** cleanup groups and callback environments define cancellation, priority, affinity, and teardown relationships.
3. **Constraint:** worker factories participate in lower-level process worker management but are not equivalent to every public thread-pool abstraction.
4. **Evidence:** lifetime analysis follows submission, pending state, callback start, cancellation race, callback return, object close, and module unload.
...
For **Windows Thread Pools**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **Windows Thread Pools** after observing only the first visible
