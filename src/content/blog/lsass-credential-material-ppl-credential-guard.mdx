...
description: "A mechanism-first, version-aware study of LSASS Credential Material, LSA Protection, and Credential Guard, with trust boundaries, evidence collection, a safe guided lab, telemetry, hardening, and reproducible case analysis."
locale: "en"
...
learningObjectives:
  - "Explain LSASS Credential Material, LSA Protection, and Credential Guard as a sequence of state transitions and trust boundaries"
...
The subject here is **LSASS Credential Material, LSA Protection, and Credential Guard**. The goal is not to memorize a named
...
case. In the curriculum this page is placed as windows-privesc lesson 20 in `credentials-recovery`.

## Concept in one sentence

LSASS hosts authentication packages, creates logon sessions, and brokers policy and credential operations, but not every logon type leaves the same material. This definition is deliberately operational: it names
...
1. **State:** LSASS hosts authentication packages, creates logon sessions, and brokers policy and credential operations, but not every logon type leaves the same material.
2. **Boundary:** RunAsPPL raises the bar for opening or injecting into LSASS by requiring protected-process-compatible signers.
3. **Constraint:** Credential Guard isolates selected secrets in VTL1 behind an LSA isolation boundary while LSASS remains in VTL0.
4. **Evidence:** effective protection depends on hardware, virtualization-based security, policy, boot trust, and compatibility state.
...
For **LSASS Credential Material, LSA Protection, and Credential Guard**, build the lifecycle around the four claims above. Mark
...
Assume an analyst reports **LSASS Credential Material, LSA Protection, and Credential Guard** after observing only the first visible
