...
> **Quick answer:** The boot chain moves from firmware and Secure Boot policy through Windows Boot Manager, winload, kernel initialization, Session Manager, Wininit, services, and interactive logon.
...
For **The Windows Boot Process: From UEFI to the Session Manager**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 09 in boot-architecture. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | The boot chain moves from firmware and Secure Boot policy through Windows Boot Manager, winload, kernel initialization, Session Manager, Wininit, services, and interactive logon. |
| 2. **Manager or boundary** | BCD selects loader behavior but not every setting is honored under Secure Boot or measured-boot policy. |
| 3. **Versioned constraint** | ELAM, code integrity, VBS, and hypervisor startup establish security properties before ordinary services. |
| 4. **Observable evidence** | debugging should identify the phase, component, trust input, persistent configuration, and earliest observable failure. |
...
| --- | --- |
| **VBS** | Virtualization-based security, which uses the hypervisor to isolate selected security services and data. |
...
1. **Starting state:** verify this concrete claim — The boot chain moves from firmware and Secure Boot policy through Windows Boot Manager, winload, kernel initialization, Session Manager, Wininit, services, and interactive logon.
2. **Decision boundary:** identify and observe the component or access decision
   described here — BCD selects loader behavior but not every setting is honored under Secure Boot or measured-boot policy.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — ELAM, code integrity, VBS, and hypervisor startup establish security properties before ordinary services.
4. **Proof:** demand the joined evidence described here — debugging should identify the phase, component, trust input, persistent configuration, and earliest observable failure.
...
- [The Windows Boot Process: From UEFI to the Session Manager](https://yunolay.com/windows-boot-process-uefi-to-session-manager/)

## References

1. https://yunolay.com/windows-boot-process-uefi-to-session-manager/
