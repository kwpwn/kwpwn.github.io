...
> **Quick answer:** The PEB contains user-mode process environment and loader-related state while each TEB contains per-thread state such as stack bounds, TLS, last-error information, and exception-related fields.
...
For **TEB and PEB: A Deep Dive**, start with one practical question: Which manager owns the object or state, how is it created and referenced, where is the user-to-kernel or subsystem boundary, and which facts change across Windows builds? The rest
...
The curriculum placement is windows-internals lesson 04 in processes-execution. That placement is navigation,
...
| Step | Working model |
| --- | --- |
| 1. **Owned state** | The PEB contains user-mode process environment and loader-related state while each TEB contains per-thread state such as stack bounds, TLS, last-error information, and exception-related fields. |
| 2. **Manager or boundary** | both structures are implementation details that vary by architecture and build. |
| 3. **Versioned constraint** | kernel security decisions must not trust writable user-mode copies. |
| 4. **Observable evidence** | debugging should use symbols and APIs, label native versus WoW64 views, and avoid assuming undocumented offsets. |
...
| --- | --- |
| **PEB** | User-mode process environment state used by the loader and runtime; its undocumented fields vary by architecture and build. |
| **TEB** | Per-thread user-mode state including stack bounds, TLS, last-error information, and architecture-specific fields. |
...
1. **Starting state:** verify this concrete claim — The PEB contains user-mode process environment and loader-related state while each TEB contains per-thread state such as stack bounds, TLS, last-error information, and exception-related fields.
2. **Decision boundary:** identify and observe the component or access decision
   described here — both structures are implementation details that vary by architecture and build.
3. **Required condition:** test whether the recorded build and configuration
   satisfy this constraint — kernel security decisions must not trust writable user-mode copies.
4. **Proof:** demand the joined evidence described here — debugging should use symbols and APIs, label native versus WoW64 views, and avoid assuming undocumented offsets.
...
- [TEB and PEB: A Deep Dive](https://yunolay.com/teb-and-peb-deep-dive/)

## References

1. https://yunolay.com/teb-and-peb-deep-dive/
